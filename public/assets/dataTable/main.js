...
var Main = function() {
	...
	var runSaveSetting = function() {
		$('.save_style').on('click', function() {
			var crm200Setting = new Object;
			if($('body').hasClass('rtl')) {
				crm200Setting.rtl = true;
			} else {
				crm200Setting.rtl = false;
			};
			if($('body').hasClass('layout-boxed')) {
				crm200Setting.layoutBoxed = true;
				$("body[class]").filter(function() {
					var classNames = this.className.split(/\s+/);
					for(var i = 0; i < classNames.length; ++i) {
						if(classNames[i].substr(0, 9) === "bg_style_") {
							crm200Setting.bgStyle = classNames[i];
						}
					}

				});
			} else {
				crm200Setting.layoutBoxed = false;
			};
			if($('body').hasClass('header-default')) {
				crm200Setting.headerDefault = true;
			} else {
				crm200Setting.headerDefault = false;
			};
			if($('body').hasClass('footer-fixed')) {
				crm200Setting.footerDefault = false;
			} else {
				crm200Setting.footerDefault = true;
			};
			if($('#skin_color').attr('rel') == 'stylesheet') {
				crm200Setting.useLess = false;
			} else if($('#skin_color').attr('rel') == 'stylesheet/less') {
				crm200Setting.useLess = true;
				crm200Setting.baseColor = $('.color-base').val();
				crm200Setting.textColor = $('.color-text').val();
				crm200Setting.badgeColor = $('.color-badge').val();
			}
			;
			crm200Setting.skinClass = $('#skin_color').attr('href');

			$.cookie("crm200-setting", JSON.stringify(crm200Setting));
	...
	var runCustomSetting = function() {
		if($.cookie("crm200-setting")) {
			var loadSetting = jQuery.parseJSON($.cookie("crm200-setting"));
			...
			if($('#style_selector').length) {
				if(loadSetting.useLess) {
					...
					$('.color-badge').val('#c96076').next('.dropdown').find('i').css('background-color', '#c96076');
